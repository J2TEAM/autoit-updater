...
if( ! isset($_SESSION['_sfm_allowed'])) {
	...
	echo '<!DOCTYPE html><html><body><form action=? method=post>PASSWORD: <input type=password name=p autofocus required></form></body></html>'; 
	exit;
}
...
function showable($file) {
	return ( ! in_array($file, array(basename(__FILE__), 'download.php', 'version.php')));
}
	...
	switch ($action) {
		...
		case 'list':
			if (is_dir($file)) {
			    ...
			    foreach($files as $entry) if (showable($entry)) {
		    		$i = $directory . '/' . $entry;
			    	$stat = stat($i);
			        $result[] = array(
			        	'mtime' => $stat['mtime'],
